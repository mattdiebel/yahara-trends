...
INFO = readNWISInfo("05427910", "00665", interactive=FALSE)
INFO$shortName = "Sixmile Creek"
INFO$staAbbrev = "SXM"
INFO$paramShortName = "Total Phosphorus"
INFO$constitAbbrev = "TP"

Sample = readNWISSample("05427910", "00665")
Sample = Sample[Sample$Date>=as.Date("2012-10-01"),]
...
write.csv(opd, file="Sixmile_samples.csv", row.names = FALSE)
Sample = readUserSample(filePath="C:/LWRD/yahara-trends/QSM/", fileName="Sixmile_samples.csv")

Daily = readNWISDaily("05427948", "00060", "2012-10-01", "2019-09-04")
...
plot_title = paste0("Sixmile Creek WRTDS","\nR-squared = ", R2, ", RMSE = ", RMSE)
lims = c(min(Sample$ConcHat,Sample$ConcAve),max(Sample$ConcHat,Sample$ConcAve))
png("Sixmile_WRTDS_fit_obs.png", res=300, height=1800, width=1800)
...
R2 = round(summary(lm)$r.squared,2)
plot_title = paste0("Sixmile Creek QSM","\nR-squared = ", R2, ", RMSE = ", RMSE)
lims = c(min(Sample$ConcHat_QSM,Sample$ConcAve),max(Sample$ConcHat_QSM,Sample$ConcAve))
png("Sixmile_QSM_fit_obs.png", res=300, height=1800, width=1800)
