...
library(lubridate)
source("~/Dropbox/BFM/runBaseReg.R") # this is the function that does base flow model estimation
load("~/Dropbox/BFM/PotomacCB.TN.RData") # this file must contain eList, modelEstimation must already have been run before the workspace is saved
eList$INFO # this command allows us to see the contents of the metadata
savePath <- "~/Dropbox/BFM/"   # needs to be specified, the place where the workspace gets saved
...
Qdf <- read.csv("~/Dropbox/BFM/flowsep.ChBridge.csv")
cat("\nsummary(Qdf)\n")
